...
class Board_post extends CB_Controller
{
    ...
    public function lists($brd_key = 'vtn_karaoke')
    {
        ...
        $group_id = $this->board->item_key('bgr_id', $brd_key);
    ...
    public function _get_list($brd_key, $from_view = '')
    {
        ...
        $main_result = $this->Post_model
            ->get_post_list(6, '', $where, $category_id, $findex, $sfield, $skeyword,'',$where_in);
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                $result['list'][$key]['modify_url'] = modify_url(element('post_id', $val));
        ...
        $menu = $this->Menu_model->get_all_menu($this->cbconfig->get_device_view_type());
        $curentContents='';
        $y=0;
        if ($menu) {
                if (element(0, $menu)) {
                    foreach (element(0, $menu) as $mkey => $mval) {
                        if(strpos($mval['men_link'],$brd_key) !==false) {
                            $curentContents=$y;
                        }
                        $y++;
                    }
        ...
        if($brd_key==='vtn_review' || $brd_key==='vtn_free') $config['base_url'] = board_url($brd_key) . '?' . $param->replace('page');
        else $config['base_url'] = 'main?curentContents='.$curentContents.'&'. $param->replace('page');
